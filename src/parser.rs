...
impl PedroParser<'_> {
    pub fn parse<'a>(input: &'a str, cl_fn_builder: FunctionBuilder<'a>, pedro_libc: HashMap<PedroLibC, FuncRef>) -> FunctionBuilder<'a> {
    ...
    fn parse_var_assign(&mut self, pair: Pair<Rule>) {
        ...
        let cl_val = self
            ...
            self
            .cl_fn_builder
            .def_var(pedro_var.cl_variable, cl_val);
    }
            ...
            for p in pairs.next().unwrap().into_inner() {
                match p.as_rule() {
                    ...
                    Rule::identifier => {
                        ...
                        let cl_val =
                        self.cl_fn_builder.use_var(pedro_var.cl_variable);
                        self.cl_fn_builder.ins().call(*self.pedro_libc.get(&PedroLibC::PutChar).unwrap(), &[cl_val]);
                    }
                    _ => {}
                }
            }
